...
// Renaud: comment following lines causing troubles when linking with share libraries
//extern "C" {
...
class PDFTool {
 public:
  ...
  double testPdf(std::string pdfname, int member, double x, double q, int pdgid){
    ...
    // Renaud: replace Fortran interface by new LHAPDF.h C++ interface
    double f[13];
  ...
  int setPdf(std::string pdfname, int member){
    for(unsigned int i=0; i<m_pdfsetNames.size(); i++){
      if(m_pdfsetNames[i]==pdfname) { // && m_pdfsetMembers[i]==member) {
#ifdef USEPDFTOOL
