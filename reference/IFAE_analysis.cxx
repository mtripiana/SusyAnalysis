...
void chorizo :: bookTree(){
  ...
  if (output){
    ...
    if(doAnaTree){ //AnalysisTree
      ...
      m_atree->Branch("m_isoT",&m_isoTight);
      m_atree->Branch("m_isoL",&m_isoLoose);
      m_atree->Branch("m_isoLTO",&m_isoLooseTrackOnly);
      m_atree->Branch("m_isoG",&m_isoGradient);
      m_atree->Branch("m_isoGL",&m_isoGradientLoose);
      m_atree->Branch("m_d0_sig",&m_d0_sig);  
      m_atree->Branch("m_z0",&m_z0);    
...
EL::StatusCode chorizo :: initialize ()
{
  ...
  if (this->isMC){
    if(genPUfile && !doPUTree){ //--- Generate the pileup root files
      ...
      Info("initialize()", Form("Reading PileupReweighting file : %i.root",  eventInfo->mcChannelNumber()) );

      TString prwfile=PURW_Folder+Form("%i",  eventInfo->mcChannelNumber())+".root";
...
EL::StatusCode chorizo :: loop ()
{
  ...
  if (EventNumber==11338 && RunNumber==222510){
     cout << "Arely found an evil event...Arely will delete it *evil laugh* (o_O)" << endl;   
     return nextEvent();
  }
  ...
  if (genPUfile && isMC){
    ...
    if (pileup_w == TMath::Infinity()) pileup_w=0.;
    return nextEvent();
  }
  ...
  if (isMC){
    ...
    if(isMC && applyPURW)
       ...
       if (pileup_w==TMath::Infinity()) pileup_w=0.;
       //if (isMC) pileup_w = tool_purw->GetCombinedWeight(222510/222525, 410000, averageIntPerXing);
...
*/
  ...
  if(m_skim_met || m_skim_btag){
    ...
    bool SK_passMu = (mb_N>0);

    bool keep_event = ( (m_skim_btag && SK_passBtagging && (SK_passSR || SK_passCRe || SK_passCRmu || SK_passCRph)) || (m_skim_met && (SK_passAnyMet||SK_passMu)) );
...
void chorizo :: dumpLeptons(){
  ...
  for(unsigned int imu = 0; imu < BookMuSignal; imu++){
    ...
    m_d0_sig.push_back( fill ?  recoMuons.at(imu).d0_sig  : DUMMYDN );
    m_z0.push_back( fill ?  recoMuons.at(imu).z0  : DUMMYDN );
